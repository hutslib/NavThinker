...
class PolicyActionData:
    ...
    lookahead_features: Optional[torch.Tensor] = None
...
class NetPolicy(nn.Module, Policy):
    ...
    def act(
        ...
        # Scheme B+: now that action_t is chosen, write (obs_t, action_t)
        # into the WM history buffer so every frame is correctly paired.
        la_embed = aux_loss_state.get("_la_embed_seq")
        if la_embed is not None:
            la_is_first = aux_loss_state["_la_is_first"]
            net = self.net
            if hasattr(net, "world_model") and net.world_model is not None:
                num_act = (
                    net.prev_action_embedding.num_embeddings - 1
                    if hasattr(net, "prev_action_embedding")
                    else action.shape[-1]
                )
                chosen_oh = torch.zeros(
                    action.shape[0], num_act, device=action.device
                )
                chosen_oh.scatter_(1, action.long().view(-1, 1), 1.0)
                with torch.no_grad():
                    net.world_model.dynamics.observe_post_action(
                        la_embed, chosen_oh, la_is_first
                    )
        ...
        return PolicyActionData(
            ...
            lookahead_features=aux_loss_state.get("lookahead_feature_for_storage", None),
        )
