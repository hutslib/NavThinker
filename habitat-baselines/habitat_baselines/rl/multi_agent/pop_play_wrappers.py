...
class MultiAgentPolicyActionData(PolicyActionData):
    ...
    length_lookahead_features: Optional[torch.Tensor] = None
    ...
    def unpack(self):
        ...
        return {
            ...
            "lookahead_features": (
                None
                if self.lookahead_features is None
                else self._unpack(
                    self.lookahead_features, self.length_lookahead_features
                )
            ),
...
class MultiPolicy(Policy):
    ...
    def act(
        ...
        **kwargs, 
        ...
        la_feature_lengths = [
            0 if ac.lookahead_features is None else ac.lookahead_features.shape[-1]
            for ac in agent_actions
        ]
        merged_la_features = (
            None
            if sum(la_feature_lengths) == 0
            else _maybe_cat(
                lambda ac: ac.lookahead_features,
                la_feature_lengths,
                torch.float32,
            )
        )
        return MultiAgentPolicyActionData(
            ...
            lookahead_features=merged_la_features,
            length_lookahead_features=la_feature_lengths,
            num_agents=n_agents,
        )
