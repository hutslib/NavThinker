...
class RolloutStorage(Storage):
    ...
    def insert(
        ...
        **kwargs,
        ...
        if lookahead_features is not None and "lookahead_features" not in self.buffers:
            self.buffers["lookahead_features"] = torch.zeros(
                self.num_steps + 1,
                self._num_envs,
                lookahead_features.shape[-1],
                dtype=lookahead_features.dtype,
                device=lookahead_features.device,
            )
    ...
    def data_generator(
        ...
        for inds in torch.randperm(num_environments).chunk(num_mini_batch):
            ...
            if "lookahead_features" in batch:
                batch["observations"]["lookahead_cached_feature"] = batch[
                    "lookahead_features"
                ]
